  ...
  context 'when errand jobs are used' do
    ...
    context 'when errand has been run with --keep-alive' do
      it 'immediately updates the errand job' do
        output = bosh_runner.run('download manifest simple')
        puts output
        ...
        }.to raise_error(nil, "Failed to find vm dead-errand/0")
      end
    end

    context 'when errand is not running' do
      it 'should not be created during deploy' do
        # @todo
      end
    end
  end
