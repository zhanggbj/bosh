...
describe 'run errand success', type: :integration, with_tmp_dir: true do
  ...
  context 'when errand script exits with 0 exit code' do
    ...
    context 'when --download-logs is not passed' do
      before(:all) do
        @output, @exit_code = bosh_runner.run("run errand fake-errand-name",
                                              {return_exit_code: true})
      end

      it 'display all the contents of log files with their names match *.log' do
        ...
        expect(@output).to_not include("/errand1/stdout.log.1")
      end
      ...
      it 'does not explicitly download errand logs and does not show downloaded location' do
        expect(@output =~ /Logs saved in `(.*fake-errand-name\.0\..*\.tgz)'/).to(be_nil, @output)
      end
    ...
    context 'when --download-logs is passed' do
      before(:all) do
