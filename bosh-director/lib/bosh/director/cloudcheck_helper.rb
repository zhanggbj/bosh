  ...
  module CloudcheckHelper
    ...
    def recreate_vm(vm)
      ...
      deployment_plan_from_model = DeploymentPlan::Planner.new(
        ...
        deployment_model,
        {'recreate' => true})
      empty_job_to_satisfy_instance = DeploymentPlan::Job.new(deployment_plan_from_model, @logger)
      empty_job_to_satisfy_instance.name = 'empty-job-to-satisfy-instance'

      empty_job_to_satisfy_instance.vm_type = DeploymentPlan::VmType.new(vm.apply_spec['vm_type'])
      empty_job_to_satisfy_instance.env = DeploymentPlan::Env.new(vm.env)
      stemcell = DeploymentPlan::Stemcell.new(vm.apply_spec['stemcell'])

      stemcell.add_stemcell_model
      empty_job_to_satisfy_instance.stemcell = stemcell
      ...
      instance_with_empty_job_and_deployment = DeploymentPlan::Instance.new(empty_job_to_satisfy_instance, instance_model.index, instance_model.state, deployment_plan_from_model, nil, availability_zone, instance_model.bootstrap, @logger)
      instance_with_empty_job_and_deployment.bind_existing_instance_model(instance_model)

      instance_plan_to_create = DeploymentPlan::InstancePlan.new(
        existing_instance: instance_model,
        instance: instance_with_empty_job_and_deployment,
