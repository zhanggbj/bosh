    ...
    describe '#parse' do
      ...
      describe 'with deployment' do
        ...
        context 'when runtime manifest has an include section' do
          let(:runtime_manifest) { Bosh::Spec::Deployments.runtime_config_with_addon_includes }

          it 'only includes addons on jobs specified in the include section' do
            ...
            expect(deployment.job('dummy').templates.map(&:name)).to eq(['dummy', 'dummy_with_properties'])
          end

          it 'excludes addons on jobs that are not specified in the include section' do
            ...
            expect(deployment.job('dummy').templates.map(&:name)).to eq(['dummy'])
          end
        end
