    ...
    describe 'recreate_vm resolution' do
      ...
      context 'when no errors' do
        ...
        before do
          ...
          allow(AgentClient).to receive(:with_vm_credentials_and_agent_id).with(@instance.credentials_json, 'agent-222', anything).and_return(fake_new_agent)
          allow(AgentClient).to receive(:with_vm_credentials_and_agent_id).with(@instance.credentials_json, 'agent-222').and_return(fake_new_agent)
