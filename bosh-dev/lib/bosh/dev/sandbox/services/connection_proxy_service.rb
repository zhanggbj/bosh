...
module Bosh::Dev::Sandbox
  class ConnectionProxyService
    def initialize(sandbox_root, forward_to_host, forward_to_port, listen_port, logger)
      @logger = logger

      config_path = File.join(sandbox_root, 'nginx_tcp_proxy.conf')
      @process = Service.new(%W[#{TCPProxyNginx.new.executable_path} -c #{config_path}], {}, logger)
      puts %W[#{TCPProxyNginx.new.executable_path} -c #{config_path}].join(' ')
